class Api::V1::UsersController < ApplicationController
    def getUser
        @authenticated_user = User.find(@authenticated_user_id)
        @data_to_send = {
    ...
    def deleteUser
        ...
        if @authenticated_user && @authenticated_user.employee_id == params[:employee_id] && @authenticated_user.password == params[:password]
            ...
            render json: {status:"error",error: {message: "invalid employee_id or password"}}
        end
    end
    ...
    def editUser
        ...
        if @authenticated_user && @authenticated_user.employee_id == params[:employee_id]
            ...
            render json: {status:"error",error: {message: "not allowed"}}
        end
    end
end
