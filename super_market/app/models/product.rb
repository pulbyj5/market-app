class Product < ApplicationRecord
    ...
    validates :batch, format: { with: /\A[a-z A-Z0-9]+\z/, message:"Enter a valid batch"}, presence: true,length: { minimum: 2, maximum: 40 ,message: "Batch must be 2 to 40 characters long"}
   validates :brand, format: { with: /\A[a-z A-Z]+\z/, message:"Enter a valid brand"}, presence: true,length: { minimum: 2, maximum: 40 ,message: "brand must be 2 to 40 characters long"}
