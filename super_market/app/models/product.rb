class Product < ApplicationRecord
    ...
    validates :name, format: { with: /\A[a-z _.,A-Z0-9]+\z/, message:"Enter a valid name"}, presence: true,length: { minimum: 2, maximum: 40 ,message: "name must be 2 to 40 characters long"}
    validates :batch, format: { with: /\A[a-z _.,A-Z0-9]+\z/, message:"Enter a valid batch"}, presence: true,length: { minimum: 2, maximum: 40 ,message: "Batch must be 2 to 40 characters long"}
